...
#include "recycledTimeVaryingMappedInletOutletFvPatchField.H"
#include "Time.H"
...
template<class Type>
Foam::fileName
Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::findFieldFile
(
    const word& timeName
) const
{
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::checkTable()
{
    // Initialise
    ...
    if (mapperPtr_.empty())
    {
        ...
        if (debug)
        {
            Info<< "recycledTimeVaryingMappedInletOutletFvPatchField :"
        ...
        if (debug)
        {
            Info<< "recycledTimeVaryingMappedInletOutletFvPatchField : In directory "
...
template<class Type>
Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::
recycledTimeVaryingMappedInletOutletFvPatchField
(
    const fvPatch& p,
...
:
    ...
    window_(dict.lookupOrDefault("window", 1.0))
{
...
recycledTimeVaryingMappedInletOutletFvPatchField
(
    const recycledTimeVaryingMappedInletOutletFvPatchField<Type>& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
template<class Type>
const Foam::mappedPatchBase& Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::mapper
(
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF
)
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::autoMap
(
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::rmap
(
...
)
{
    ...
    const recycledTimeVaryingMappedInletOutletFvPatchField<Type>& tiptf =
        refCast<const recycledTimeVaryingMappedInletOutletFvPatchField<Type>>(ptf);
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    // Now that the inflowOutflow and mapped BD data have been updated, grab
    // the upstream turbulence information from the mapping plane and append
    // the perturbation onto the new time-varying mapped inflow.

    // Grab the upstream information
    Field<Type> recycledField = this->mappedField();

    //Initialize the recycledFieldAvg with the first sampled value
    if (this->db().time().value() == this->db().time().startTime().value())
    {
        recycledFieldAvg = recycledField;    
    }
    else
    {
        //Begin calculating the Gaussian-weighted backward time average
        scalar dt = this->db().time().deltaTValue();
        scalar beta = dt/window_;
        recycledFieldAvg = (1 - beta)*recycledFieldAvg + beta*recycledField;
    }

    // Subtract instantaneous from mean to get perturbation
    Field<Type> perturbation = recycledField - recycledFieldAvg;

    // Add the upstream perturbation onto the new inflow
    this->refValue() += perturbation;
}


template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::updateInletOutlet()
{
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::updateFixedValue()
{
    checkTable();
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::evaluate(
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::write
(
    Ostream& os
) const
{
...
template<class Type>
void Foam::recycledTimeVaryingMappedInletOutletFvPatchField<Type>::operator=
(
    const fvPatchField<Type>& ptf
)
{
